...
def regression(tmodel,m, mvars,conds, flux_data, metabolite_data, volume_data,
    ...
    if flux_fit:
        ...
        measured_fluxes = [rxn for rxn in measured_fluxes if rxn in reaction_id]
    ...
    if conc_fit:
        ...
        metabolite_id = ["_".join(met.id.split('_')[:-1]) for met in tmodel.metabolites]
        measured_mets = [met for met in measured_mets if met in metabolite_id]
    ...
    for cond_index, cond in enumerate(conds):
        ...
        if conc_fit:
            ...
            for i, row in enumerate(dmetsmeas.iterrows()):
                met_id = row[1].name
                if met_id not in metabolite_id:
                    print('Warning: metabolite {} not found in model, skipping'.format(met_id))
                    # Skip metabolites not in the model
                    continue
                if met_id not in measured_mets:
                    print('Warning: metabolite {} not found in measured metabolites, skipping'.format(met_id))
                    # Skip metabolites not in measured_mets
                    continue
        ...
        if flux_fit:
            ...
            for i, row in enumerate(dvmeas.iterrows()):
                ...
                flux = row[1]['mean']
                sd = row[1]['sd']
                if rxn_id not in reaction_id:
                    print('Warning: reaction {} not found in model, skipping'.format(rxn_id))
                    # Skip reactions not in the model
                    continue
                if rxn_id not in measured_fluxes:
                    print('Warning: reaction {} not found in measured fluxes, skipping'.format(rxn_id))
                    # Skip reactions not in measured_fluxes
                    continue
